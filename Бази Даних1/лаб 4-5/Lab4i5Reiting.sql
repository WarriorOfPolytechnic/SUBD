create database Lab4i5_reiting
go
USE [Lab4i5_reiting] 
GO
...
SELECT dbo_groups.Kod_group, COUNT(dbo_student.Name) AS CountStuden
FROM dbo_groups INNER JOIN dbo_student ON dbo_student.Kod_group = dbo_groups.Kod_group
GROUP BY dbo_groups.Kod_group;



/*Розрахувати кількість дисциплін за групою.*/
SELECT q.Kod_group, COUNT(q.Nazva) AS CountSubjects FROM (SELECT DISTINCT dbo_groups.Kod_group,  predmet.Nazva 
FROM (dbo_groups INNER JOIN Rozklad_pids ON Rozklad_pids.Kod_group = dbo_groups.Kod_group)
...
SELECT Rozklad_pids.Kod_group, Count(Rozklad_pids.K_zapis) AS CountLessons
from Rozklad_pids
GROUP BY Rozklad_pids.Kod_group;


/*Розрахувати середній бал за групою.*/
SELECT dbo_groups.Kod_group, AVG(Reiting.Reiting) AS СередБал
FROM (dbo_groups INNER JOIN dbo_student ON dbo_student.Kod_group = dbo_groups.Kod_group)
INNER JOIN Reiting ON Reiting.Kod_student = dbo_student.Kod_stud
GROUP BY dbo_groups.Kod_group;
...
SELECT
  predmet.Nazva,
  AVG(Reiting.Reiting) AS СередБал
FROM
  predmet
...
SELECT predmet.Nazva, Max(Reiting.Reiting) AS MaxReiting
FROM predmet
INNER JOIN Predmet_plan ON Predmet_plan.K_predmet = predmet.K_predmet
INNER JOIN Rozklad_pids ON Rozklad_pids.K_predm_pl = Predmet_plan.K_predm_pl
INNER JOIN Reiting ON Reiting.K_zapis = Rozklad_pids.K_zapis
...
/*Запит на знищення даних з таблиці «Reiting» за визначеним кодом студента (в поле параметра вводиться прізвище студента).*/
DELETE r from Reiting r INNER JOIN dbo_student s ON r.Kod_student='55' WHERE s.Sname='Маляренко';
...
update Reiting set Reiting.Reiting = (Reiting.Reiting * 0.75)
     where Reiting.K_zapis in (select Rozklad_pids.K_zapis from Rozklad_pids
     inner join Predmet_plan on Predmet_plan.K_predm_pl = Rozklad_pids.K_predm_pl
     inner join form_kontr on form_kontr.k_fk = Predmet_plan.k_fk);

/*Запит на вставку даних до таблиці «Reiting» – передбачити вставку даних студентів визначеної групи (код пари та початковий бал задається динамічно).*/
 insert into Reiting (Kod_student, Reiting) 
    values ('kod_stud', 'bal');

/*Запит на оновлення даних – передбачити зміну порядкового номера змістовного модуля за певною дисципліною на нове значення.*/
   update Rozklad_pids set Rozklad_pids.N_vedomost = Null
    where Rozklad_pids.K_predm_pl in (
    select Predmet_plan.K_predm_pl from Predmet_plan
      inner join predmet on predmet.K_predmet = Predmet_plan.K_predmet
       where predmet.Nazva = 'Системи технічного захисту інформації');
...
/*Запит на вставку даних до таблиці «Reiting» – передбачити вставку даних студентів визначеної групи (код пари та присутність задаються динамічно) */
insert into Reiting (Kod_student, Prisutn, K_zapis) 
    select dbo_student.Kod_stud, prisutn, k_zapis from dbo_student 
    inner join dbo_groups on dbo_groups.Kod_group = dbo_student.Kod_group;

/*Передбачити оновлення даних у таблиці «Reiting»  поле «Prisutnist», для студента із визначеним прізвищем автоматично встановлюється true.*/
	update Reiting set Reiting.Prisutn = 1
    where Reiting.Kod_student in (select dbo_student.Kod_stud from dbo_student where dbo_student.SName='Кравченко');