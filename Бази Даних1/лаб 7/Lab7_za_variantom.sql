Create Database Lab7Studentss; 

create table Lab7Studentss.dbo.kafedra
 (id_kafedru int IDENTITY(1,1) not null primary key,
  ...
  create table Lab7Studentss.dbo.Specialnist
 (id_specialnosti int IDENTITY(1,1) not null primary key,
  ...
  create table Lab7Studentss.dbo.grupa
 (id_group int IDENTITY(1,1) not null primary key,
  ...
  create table Lab7Studentss.dbo.Pruchuna_vidrahuvanna
 (id_pruchuna_vidrah int IDENTITY(1,1) not null primary key,
   nazva_vidrah varchar(50) constraint  nazva_vidrah_format check (nazva_vidrah like '[А-я]%'))
    ...
    create table Lab7Studentss.dbo.Student
 ...
 Insert into
  Lab7Studentss.dbo.kafedra (nazva_kafedru,name_zav_kafedru,surname_zav_kafedru,middlename_zav_kafedru,kabinet,telefon)
 ...
 ('Кафедра прикладной механики и компьютерно-интегрированных технологий, мехатроники и робототехники', 'Елена', 'Остапенко', 'Петровна', '417', '(063)7645142')
   ...
   Insert into
 Lab7Studentss.dbo.Pruchuna_vidrahuvanna (nazva_vidrah)
 Values ('По собственному желанию'),
 ('За академическую неуспеваемость'),
 ('По завершению обучения')

 Insert into
  Lab7Studentss.dbo.Specialnist (nazva_specialnosti,shufr_specialnosti,kvalifikacia, id_kafedru)
 ...
 ('Компьютерные науки', '122', 'Специалист по информационным технологиям', '2')
  ...
  Insert into
  Lab7Studentss.dbo.grupa (shufr_group,forma_navchanna, id_specialnosti)
 Values ('ПМ-20-1', 'Дневная','4' ),
 ('ОМ-4', 'Заочная', '2'),
 ('КН-19-2', 'Дневная', '3')

  Insert into
   Lab7Studentss.dbo.Student (name_st,surname_st,middlename_st,datanarod_st,adresa_st,tel_st,rik_vstupu_st,byudzhet_contrack,nom_zalikovki,data_vidrahuvanna, id_pruchuna_vidrah,id_group)
...
create nonclustered index ix_Name on Specialnist(shufr_specialnosti)
with fillfactor = 70
select OBJECT_NAME(object_id) as table_name,
name as index_name, type, type_desc
from sys.indexes
where OBJECT_ID = OBJECT_ID(N'Specialnist')

drop index ix_Student_id_id_group on Student
create nonclustered index ix_Student_id_id_group on Student(id_pruchuna_vidrah, id_group)
select OBJECT_NAME(object_id) as table_name,
name as index_name, type, type_desc
from sys.indexes
where OBJECT_ID = OBJECT_ID(N'Student')

create nonclustered index ix_Student_id_id_group2 on Student(id_group,id_pruchuna_vidrah)
select OBJECT_NAME(object_id) as table_name,
name as index_name, type, type_desc
from sys.indexes
where OBJECT_ID = OBJECT_ID(N'Student')
...
(db_id(N'StudentStudent'), OBJECT_ID(N'Student'), Null, Null, 'Detailed')


create nonclustered index ix_pruchuna_vidrahuvanna11 on Pruchuna_vidrahuvanna(nazva_vidrah)
select * from Pruchuna_vidrahuvanna
where nazva_vidrah like 'За академическую неуспеваемость'

create nonclustered index ix_faculty__name_faculty_code on Pruchuna_vidrahuvanna(nazva_vidrah)
select * from Pruchuna_vidrahuvanna
where nazva_vidrah = 'За академическую неуспеваемость' and id_pruchuna_vidrah = 2
