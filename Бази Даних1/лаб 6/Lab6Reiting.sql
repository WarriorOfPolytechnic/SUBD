...
/*Розрахунок середнього балу студентів за період – без урахування перездач*/
CREATE PROCEDURE SeredBalZaPeriod1 AS
BEGIN
  select AVG(r.Reiting), s.SName, s.Name, s.FName from Reiting r    
  INNER JOIN Rozklad_pids rp ON (r.K_zapis=rp.K_zapis) 
  INNER JOIN dbo_student s ON s.Kod_stud=r.Kod_student
  WHERE Date BETWEEN '2017-01-01' AND '2017-10-01' 
  GROUP BY Sname, Name, Fname
  HAVING COUNT(*)<2;
END;
EXEC SeredBalZaPeriod1


/*Розрахунок середнього балу студентів за період – з урахування перездач.*/
 CREATE PROCEDURE SeredBalZaPeriod2 AS
BEGIN
  select AVG(r.Reiting), s.SName, s.Name, s.FName from Reiting r 
  INNER JOIN Rozklad_pids rp ON (r.K_zapis=rp.K_zapis) 
  INNER JOIN dbo_student s ON s.Kod_stud=r.Kod_student
  WHERE Date BETWEEN '2017-01-01' AND '2017-10-01' 
  GROUP BY Kod_student, Sname, Name, Fname;
  END;
  EXEC SeredBalZaPeriod2

  /*Визначення студентів, що навчаються на 4 та 5.*/
  CREATE PROCEDURE OcinkiBilshe4i5 AS
BEGIN
  select s.Sname,s.Name,s.Fname,r.Reiting from dbo_student s 
  INNER JOIN Reiting r ON r.Kod_student=s.Kod_stud 
  WHERE Reiting>=74;
  END;
  EXEC OcinkiBilshe4i5

  /*Процедура виводить суму балів та її значення в національній системі та ECTS.*/
  CREATE PROC OcinkaNationalAandECTS AS
	SELECT R.Name_ini,
	CASE
			WHEN R.AverageReitingWithRetakes > 90 THEN 'Відмінно'
			WHEN R.AverageReitingWithRetakes > 80 THEN 'Добре'
			WHEN R.AverageReitingWithRetakes > 65 THEN 'Добре'
			WHEN R.AverageReitingWithRetakes > 55 THEN 'Задовільно'
			WHEN R.AverageReitingWithRetakes > 50 THEN 'Задовільно'
			WHEN R.AverageReitingWithRetakes > 35 THEN 'Незадовільно'
			WHEN R.AverageReitingWithRetakes > 0 THEN 'Незадовільно'
	END NationalReiting,
	CASE
...
END ECTS
		FROM (SELECT S.Name_ini, AVG(R.Reiting) as AverageReitingWithRetakes FROM
		dbo_student S
...
EXEC OcinkaNationalAandECTS

/*Тригер на вставку даних в таблицю студент – якщо код групи новий в таблицю додається група.*/
CREATE TRIGGER Insert_Student ON dbo_student INSTEAD OF INSERT AS
...
/*Тригер на модифікацію даних з таблиці  студенти якщо більше немає студентів в групі група знищується.*/
CREATE TRIGGER Update_STUDENT ON dbo_student AFTER UPDATE AS
	IF EXISTS(SELECT * FROM dbo_groups G
		LEFT JOIN dbo_student S ON S.Kod_group = G.Kod_group
		WHERE Kod_stud IS NULL)
	BEGIN
